<!DOCTYPE html>
<html dir="@if (session()->has('language_direction_from_dropdown')) @if(session()->get('language_direction_from_dropdown') == 1) {{ __('rtl') }} @else {{ __('ltr') }} @endif @else {{ __('ltr') }} @endif"
...
<head>
    ...
    <meta name="title" content="@if (!empty($service->title)) {{ $service->title }} @elseif (!empty($page_builder->meta_title)) {{ $page_builder->meta_title }} @elseif (isset($seo)){{ $seo->meta_title }} @endif">
    <meta name="description" content="@if (!empty($service_content->meta_description)) {{ $service_content->meta_description }} @elseif (!empty($page_builder->meta_description)) {{ $page_builder->meta_description }} @elseif (isset($seo)){{ $seo->meta_description }} @endif">
    <meta name="keywords" content="@if (!empty($service_content->meta_keyword)) {{ $service_content->meta_keyword }} @elseif (!empty($page_builder->meta_keyword)) {{ $page_builder->meta_keyword }} @elseif (isset($seo)){{ $seo->meta_keyword }} @endif">
    <meta name="author" content="elsecolor">
    <meta property="fb:app_id" content="@if (isset($seo)){{ $seo->fb_app_id }} @endif">
    <meta property="og:title" content="@if (!empty($service->title)) {{ $service->title }} @elseif (!empty($page_builder->meta_title)) {{ $page_builder->meta_title }} @elseif (isset($seo)){{ $seo->meta_title }} @endif">
    <meta property="og:url" content="@if (isset($seo) || isset($page_builder)){{ url()->current() }} @endif">
    <meta property="og:description" content="@if (!empty($service_content->meta_description)) {{ $service_content->meta_description }} @elseif (!empty($page_builder->meta_description)) {{ $page_builder->meta_description }} @elseif (isset($seo)){{ $seo->meta_description }} @endif">
    ...
    <meta property="twitter:title" content="@if (!empty($service->title)) {{ $service->title }} @elseif (!empty($page_builder->meta_title)) {{ $page_builder->meta_title }} @elseif (isset($seo)){{ $seo->meta_title }} @endif">
    <meta property="twitter:description" content="@if (!empty($service_content->meta_description)) {{ $service_content->meta_description }} @elseif (!empty($page_builder->meta_description)) {{ $page_builder->meta_description }} @elseif (isset($seo)){{ $seo->meta_description }} @endif">

    <!-- CSRF Token -->
    <meta name="csrf-token" content="{{ csrf_token() }}">

    <!-- Title -->
    <title>@if (!empty($service->title)) {{ $service->title }} @elseif (!empty($page_builder->meta_title)) {{ $page_builder->meta_title }} @elseif  (isset($seo)) {{ $seo->meta_title }} @endif</title>
...
@include('frontend.service.service-detail')
@isset ($fixed_page_setting)
    @if ($fixed_page_setting->subscribe_section == 'enable')
        @include('frontend.sections.subscribe.subscribe-style1')
    @endif
@endisset
@isset ($fixed_page_setting)
